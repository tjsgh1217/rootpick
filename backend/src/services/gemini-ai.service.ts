...
export class GeminiAiService {
  ...
  async generateKeywordsByAddress(
    ...
    try {
      ...
      **음식 종류**: 한식, 중식, 일식, 양식, 분식, 치킨, 피자, 햄버거, 족발, 보쌈, 삼겹살, 갈비, 냉면, 국밥, 찌개, 회, 초밥, 라멘, 우동, 파스타, 스테이크
      **업체 유형**: 맛집, 식당, 음식점, 카페, 베이커리, 술집, 포차, 횟집, 고깃집, 분식집
       
      위 예시를 참고하여 약 20개의 다양한 키워드를 생성해주세요.
      형식: - 키워드 (한 줄에 하나씩)
      `;
  ...
  async compareRestaurants(restaurants: RestaurantInsight[]): Promise<string> {
    try {
      ...
      const prompt = `다음 ${restaurants.length}개의 음식점을 비교 분석해주세요.
